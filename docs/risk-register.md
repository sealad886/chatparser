...
|---|---|---:|---:|---|---|---|
| R-001 | Voicebox API changes from documented README shape. | Medium | High | Keep Voicebox client isolated, validate `/health`, `/profiles`, `/transcribe`, `/generate` in integration tests, link endpoint metadata to artifacts. | Engineering | Monitoring |
...
| R-009 | Voicebox unavailable blocks batch work. | Medium | Medium | Queue jobs, run service preflight with `GET /health`, check profile availability with `GET /profiles`, retry with backoff, preserve job state. | Engineering | Open |
