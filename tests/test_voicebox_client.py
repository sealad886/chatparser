...
class OpenStreamingFakeResponse(FakeResponse):
    def __init__(self, payload: bytes):
        super().__init__(200, b"", {"Content-Type": "text/event-stream"})
        self._lines = iter(payload.splitlines(keepends=True))
    ...
    def readline(self) -> bytes:
        try:
            ...
            raise AssertionError("client should not wait for streaming status EOF") from None

    def read(self) -> bytes:
        raise AssertionError("streaming status should be read incrementally")


def test_default_opener_passes_timeout_as_keyword(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr("voicebox_client.urllib.request.urlopen", fake_urlopen)
    client = VoiceboxClient(base_url="http://127.0.0.1:17493", timeout=7)

    assert client.health() == {"status": "healthy"}
    assert captured == {
        "url": "http://127.0.0.1:17493/health",
        "data": None,
        "timeout": 7,
    }
...
def test_generate_speech_polls_again_when_status_stream_stays_open(tmp_path, monkeypatch):
    output_file = tmp_path / "speech.wav"
    captured = []
    status_calls = 0
    ...
    def opener(request, timeout):
        ...
        if request.full_url.endswith("/generate/gen-123/status"):
            status_calls += 1
            status = "generating" if status_calls == 1 else "completed"
            return OpenStreamingFakeResponse(f'data: {{"id":"gen-123","status":"{status}"}}\n'.encode())
        if request.full_url.endswith("/audio/gen-123"):
            return FakeResponse(200, b"wav-bytes", {"Content-Type": "audio/wav"})
        raise AssertionError(request.full_url)
